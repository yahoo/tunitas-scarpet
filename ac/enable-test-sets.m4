...
dnl
dnl prefixed as "SCARPET_" for "tunitas-scarpet"

dnl
dnl SCARPET_ENABLE_ALL_TESTS     (no arguments)
dnl
...
AC_DEFUN([SCARPET_ENABLE_ALL_TESTS], [
...
dnl SCARPET_ENABLE_NAMED_TEST_SET(name_dashes, name_underscores)
...
dnl Examples:
dnl
dnl     SCARPET_ENABLE_NAMED_TEST_SET(server)
dnl     SCARPET_ENABLE_NAMED_TEST_SET(um_what_other_tests)
dnl
...
AC_DEFUN([SCARPET_ENABLE_TEST_SET], [
    AC_REQUIRE([SCARPET_ENABLE_ALL_TESTS])
    SCARPET_ENABLE__NAMED__TEST_SET([$1-tests],
...
AC_DEFUN([SCARPET_ENABLE__NAMED__TEST_SET], [
    AC_REQUIRE([TF_ENABLE_CONFIGURE_VERBOSE])
    AC_ARG_ENABLE([$1],
